...
def load_project_env():
    ...
    if os.path.exists(env_path):
        ...
        print(
            f"[AVISO] Nenhum .env encontrado em {os.getcwd()}. Usando valores padrao."
        )
...
def extrair_termos_chave(code_content: str) -> str:
    ...
    termos = []
    for p in palavras:
        if p in {"raise", "return", "except", "try", "yield"} or (
            p[0].isupper() and len(p) > 2
        ):
            if p not in termos:
                termos.append(p)
...
def validate_chunk_with_llm(
    context_name: str, code_content: str, diff_content: str = None
):
    ...
    prompt = f"""Você é um auditor de código (Quality Gate de Arquitetura).
Sua tarefa é verificar se as ALTERAÇÕES RECENTES do código violam alguma regra descrita no CONTEXTO RAG.

DIRETRIZES DE AUDITORIA:
1. FOQUE EXCLUSIVAMENTE nas linhas MODIFICADAS ou ADICIONADAS (marcadas com '+' no diff).
   - Linhas que NÃO foram alteradas no diff NÃO devem ser auditadas.
2. Compare cada linha modificada contra as regras do CONTEXTO RAG.
   - Se uma regra do CONTEXTO RAG proíbe um padrão e esse padrão EXISTE LITERALMENTE em uma linha modificada do diff, isso é uma violação.
   - Se nenhuma linha modificada viola nenhuma regra, a decisão é PASSED.
3. VERIFICAÇÃO OBRIGATÓRIA antes de decidir FAILED:
   - Você DEVE citar o trecho EXATO (copiar e colar) da linha modificada do diff que contém a violação.
   - Você DEVE citar a regra EXATA do CONTEXTO RAG que está sendo violada.
   - Se você não conseguir apontar ambos (trecho do diff + regra do RAG), a decisão é PASSED.
4. NÃO invente trechos de código. Use APENAS o que está escrito no DIFF e no CÓDIGO abaixo.
5. Considere que o CONTEXTO RAG pode conter regras destinadas a USUÁRIOS de um framework.
   Se o código auditado é infraestrutura interna (ex: core de um framework), avalie se a regra realmente se aplica.

FORMATO OBRIGATÓRIO DE RESPOSTA:
DECISAO: [FAILED ou PASSED]
JUSTIFICATIVA: [Explicação técnica citando a regra exata do CONTEXTO RAG e o trecho exato do diff]
...
def audit_codebase(audit_all=False):
    ...
    if audit_all:
        print(
            f"Iniciando varredura completa da codebase (pasta={source_folder or 'raiz'})..."
        )
        all_passed = True
        ...
        for ext in extensions:
            ...
            if not files:
                print(
                    f"Nenhum arquivo encontrado para a extensão {ext} em '{source_folder or '.'}'."
                )
                continue
        ...
        if not diff_files_str.strip():
            print(
                "Nenhuma alteração encontrada para auditar. O repositório está limpo."
            )
            sys.exit(0)

        # Pega a lista unificada e remove duplicatas (caso um arquivo esteja em staged e unstaged)
        all_diff_files = list(
            set(f.strip() for f in diff_files_str.split("\n") if f.strip())
        )
        diff_files = [
            f for f in all_diff_files if any(f.endswith(ext) for ext in extensions)
        ]
        ...
        if source_folder:
            diff_files = [
                f
                for f in diff_files
                if f.replace("\\", "/").startswith(source_folder + "/")
                or f == source_folder
            ]
            print(
                f"[CONFIG] Filtrando alterações apenas na pasta '{source_folder}': {len(diff_files)} arquivo(s)"
            )

        if not diff_files:
            print(
                f"Nenhuma alteração em arquivos com as extensões {extensions} na pasta alvo."
            )
            sys.exit(0)
        ...
        for f_path in diff_files:
            ...
            if not content.startswith("Erro:"):
                passed = validate_chunk_with_llm(
                    f_path, content, diff_content=file_diff
                )
                if not passed:
                    all_passed = False
...
def main():
    parser = argparse.ArgumentParser(
        description="Doc-as-Code CLI: Validação de Código via IA"
    )
    ...
    subparsers.add_parser(
        "sync", help="Lê a documentação e atualiza o banco vetorial (RAG)"
    )
    ...
    audit_parser.add_argument(
        "--all",
        action="store_true",
        help="Faz uma varredura completa em todos os arquivos da codebase em vez de usar apenas o diff",
    )
